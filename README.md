...
`carrierElem` | HTMLElement | `document.body`| 水印层载体
`zIndex` | Number | `1000000` | 水印层（css z-index）
`mode` | String （`'mat'\|'cover'`）| `'mat'` | 水印放置模式，默认 `mat`，铺设在目标层底部；`cover` 模式会创建一个层，并覆盖在载体上。<br>*cover模式下为确保水印覆盖到，请检查载体宽高，mat 模式下水印的打印是非强制的，可能丢失*
...
`needClip` | Boolean | `true` | 是否裁切掉空白（margin、padding之外的空白）
...
`tuning` | Boolean | `false` | 调试模式开关，设为 `true` 会在水印下添加 canvas 画布生成的参照底图，能够观察到 `margin`、`padding`、`rotateDegree`、`baseline` 等参数的设置效果，方便调试。*目前只支持在 cover 模式下开启*
