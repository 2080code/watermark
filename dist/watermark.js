...
class WaterMark {
     ...
     */
    draft(options) {
        const { content, fontWeight, fontSize, fontFamily, fontColor, baseline, rotateDegree, margin, padding, opacity, needClip, tuning, degraded, } = options;
        ...
        if (tuning) {
            this.put(canvas.toDataURL('image/png'), Object.assign(Object.assign({}, options), { name: [options.name, 'tuning'].join('-') }));
        }
        return {
            width: canvas.width,
